...
export const ThemeToggle = ({ variant = 'icon', className = '' }) => {
    ...
    if (variant === 'row') {
        return (
            <button
                ...
                onClick={toggleTheme}
                aria-pressed={isDark}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[14px] font-bold tracking-tight text-slate-600 dark:text-slate-300 hover:text-green-700 dark:hover:text-green-400 hover:bg-green-50 dark:hover:bg-green-950 transition-all duration-150 bg-transparent border-none cursor-pointer ${className}`}
            >
    ...
    return (
        <button
            ...
            aria-pressed={isDark}
            title={label}
            className={`flex items-center justify-center w-9 h-9 rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition bg-transparent border-none cursor-pointer flex-shrink-0 ${className}`}
        >
